...
import logoParkways from '../assets/images/logo-parkways.png'
...
const Logo = () => {
  ...
  useEffect(() => {
    const text = 'Digital Parking Transformation';
    let animatedString = '';

    const animateText = () => {
      if (animatedString.length < text.length) {
        animatedString += text[animatedString.length];
      } else {
        animatedString = '';
      }

      setAnimatedText(animatedString);
      setTimeout(animateText, 300); // Waktu penundaan antara penambahan huruf dan reset
    };
  ...
  return (
     ...
     *
     */
    <Box display="flex" alignItems="center" marginLeft='-15px' gap='2px'>
      <img src={logoParkways} alt="parkways" width="50" height="50" />
      <Box>
        <Box
        sx={{
          marginBottom:'3px'
        ...
        <Box mt={isNonMobile ? undefined : '-3px'}>
          <Typography
            // variant="h6"
            sx={{ color: '#1E458C', fontWeight: 'bolder', fontSize: '9px' }}
          >
            {animatedText === '' ? 'D' : animatedText}
          </Typography>
        </Box>
      </Box>
    </Box>
